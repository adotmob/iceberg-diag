...
def test_file_count(computed_metrics):
    file_count_metric = next(m for m in computed_metrics[0] if m.name == MetricName.FILE_COUNT)
...
def test_worst_file_count(computed_metrics):
    worst_file_count = next(m for m in computed_metrics[0] if m.name == MetricName.WORST_FILE_COUNT)
...
def test_full_scan_overhead(computed_metrics):
    full_scan_overhead = next(m for m in computed_metrics[0] if m.name == MetricName.FULL_SCAN_OVERHEAD)
...
def test_worst_scan_overhead(computed_metrics):
    worst_scan_overhead = next(m for m in computed_metrics[0] if m.name == MetricName.WORST_SCAN_OVERHEAD)
...
def test_avg_file_size(computed_metrics):
    avg_file_size = next(m for m in computed_metrics[0] if m.name == MetricName.AVG_FILE_SIZE)
    assert avg_file_size.before == 18856891.733333334


def test_total_table_size(computed_metrics):
    total_table_size = next(m for m in computed_metrics[0] if m.name == MetricName.TOTAL_TABLE_SIZE)
    assert total_table_size.before == 11424235520


def test_largest_partition(computed_metrics):
    total_partition = next(m for m in computed_metrics[0] if m.name == MetricName.LARGEST_PARTITION_SIZE)
    assert total_partition.before == 3982491648


def test_total_partitions(computed_metrics):
    total_partitions = next(m for m in computed_metrics[0] if m.name == MetricName.TOTAL_PARTITIONS)
